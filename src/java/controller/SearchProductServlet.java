...
public class SearchProductServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int zCol = Integer.parseInt(collId);
        
        List<Product> lp;
        ProductDAO pd = new ProductDAO();
        if (zCol == 0) {
            lp = pd.getProductByName(name);
        } else {
            lp = pd.getProductByNameCategoryID(zCol, name);
        }
        
        request.setAttribute("searchProduct", lp);
        request.getRequestDispatcher("searchProductCategory.jsp").forward(request, response);
    }
